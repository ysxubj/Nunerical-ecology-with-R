...
library(ade4)
library(vegan)
library(gclus)  # # 这个软件包的目的在于将聚类分析结果映射到排序图中 Will be used to project clustering result on the
                # ordination plot
library(ape)
...
# PCA biplots
windows(title="PCA on fish species", 12, 6)
...
windows(title="PCA on environmental variables - scaling 1")
...
windows(title="PCA on environmental variables - scaling 2")
...
spe.CA.PL <- CA(spe)
windows(title="Species CA with CA() function", 14, 8)
...
# Biplots
windows(title="PCoA with species vectors", 14, 8)
par(mfrow=c(1,2))
...
# Biplots
windows(title="PCA using homemade function", 12, 8)
par(mfrow=c(1,2))
...
# Plots using R's generic plot() function

windows(title="PCA using homemade function - generic plot function", 12, 8)
par(mfrow=c(1,2))

# Scaling 1
# Plot objects
plot(fish.PCA$F[,1], fish.PCA$F[,2], asp=1, xlab="Axis 1", ylab="Axis 2", 
  col="blue")
# Plot variables
arrows(x0=0,y0=0, fish.PCA$U[,1], fish.PCA$U[,2], length=0.1, col="red")
# Add object numbers
text(fish.PCA$F[,1], fish.PCA$F[,2], labels=row.names(spe), pos=3, col="blue")
# Add variable names
text(fish.PCA$U[,1], fish.PCA$U[,2], labels=colnames(spe), adj=c(-0.2,0.2), 
  col="red")
# Scaling 2
plot(fish.PCA$G[,1], fish.PCA$G[,2], asp=1, xlab="Axis 1", ylab="Axis 2", 
  col="blue")
arrows(x0=0,y0=0, fish.PCA$U2[,1], fish.PCA$U2[,2], length=0.1, col="red")
text(fish.PCA$G[,1], fish.PCA$G[,2], labels=row.names(spe), pos=3, col="blue")
