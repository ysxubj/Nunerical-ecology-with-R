...
windows(title="Correlogram of substrate density")
plot(subs.correlog)
...
mite.trend.fit <- scores(mite.trend.rda2, choices=c(1,2,3), display="lc", 
  scaling=1)
windows(title="Mite Trend Surface Analysis")
par(mfrow=c(1,3))
...
windows(title="PCNM variables (transect)")
par(mfrow=c(4,2))
...
windows(title="PCNM variables (grid)",8,8)
par(mfrow=c(4,2))
...
mite.PCNM.auto$expected_Moran # Expected value of I, no spatial correlation
...
(nb.ax <- length(which(axes.test[,5] <= 0.05))) # Number of significant axes
...
(nb.ax.med <- length(which(axes.med[,5] <= 0.05)))  # Number of significant axes
...
(nb.ax.fine <- length(which(axes.fine[,5] <= 0.05))) # Number of significant axes
...
(nb.sig.PCNM <- nrow(mite.undet.PCNM.fwd)) # Number of signif. PCNM
# Identity of significant PCNMs in increasing order
...
windows(title="Multivariate variogram, mites")
...
mite.thresh.minAIC <- sapply(mite.thresh.res, function(x) min(x$best$AICc, 
  na.rm=TRUE))
min(mite.thresh.minAIC)  # Smallest AICc (best model among the 10)
which.min(mite.thresh.minAIC) # Number of the model among the 10
thresh10[which.min(mite.thresh.minAIC)]  # Truncation threshold (distance)
...
mite.f2.minAIC <- sapply(mite.thresh.f2, function(x) min(x$best$AICc, 
  na.rm=TRUE)) 
min(mite.f2.minAIC)         # Smallest AICc (best model among the 10)
(nb.bestmod <- which.min(mite.f2.minAIC))   # Number of the model among the 10
(dmax.best <- mite.thresh.f2[nb.bestmod][[1]]$all[1,2])  # Actual dmax of best model
...
windows(title="Links, mite MEM champion model")
plot.links(mite.xy, thresh=dmax.best)

# Maps of the 7 significant MEM variables
# ***************************************
windows(title="7 MEM variables - mites")
par(mfrow=c(2,4))
...
mite.MEM.axes <- scores(mite.MEM.rda, choices=c(1,2), display="lc", 
  scaling=1)
windows(title="MEM analysis of mite data")
par(mfrow=c(1,2))
...
windows(title="Maps of 7 significant MEM eigenfunctions")
par(mfrow=c(2,4))
for(i in 1:ncol(MEM.select)){
  s.value(mite.xy, MEM.select[,i], sub=sort(MEMid)[i], csub=2)
}
...
windows(title="Connectivity matrices")
par(mfrow=c(2,2))
...
# 1. Interactive:
### PUT AS COMMENTARY TO ALLOW ONE-SHOT RUN OF THE SCRIPT
# windows(title="Delaunay triangulation")
...
windows(title="Delaunay with edited links")
...
windows(title="Connectivity matrices - threshold distances")
par(mfrow=c(1,2))
...
windows(title="Barplot of MEM eigenvalues")
...
which(mite.MEM.Moran[,2] <= 0.05) # MEM with significant spatial correlation
...
windows(title="MEM eigenvalues vs Moran's I")
...
arbor.svd$d[1:5]    # Singular values of the previous construction
arbor.svd$u[,1:5]   # AEM eigenfunctions of the previous construction
...
AEM.sign.vec <- ex.AEM.vec[,c(AEM.sign)] # Write significant AEM in a new object
(sp.AEMsign.rda <- rda(sp ~ ., data=as.data.frame(AEM.sign.vec))) # RDA with signif. AEM
anova.cca(sp.AEMsign.rda)
AEM.rda.axes.test <- anova.cca(sp.AEMsign.rda, by="axis")
(nb.ax.AEM <- length(which(AEM.rda.axes.test[,5] <= 0.05))) # Number of significant axes

# Plot of the significant canonical axes
AEM.rda.axes <- scores.cca(sp.AEMsign.rda, choices=c(1,2), display="lc", 
  scaling=1)
windows(title="AEM analysis of fictitious data, significant RDA axes")
...
windows(title="MSO plot of the undetrended mite-environment RDA")
...
windows(title="MSO plot of the undetrended mite-environment RDA controlling for MEM")
...
windows(title="MSO plot of the detrended mite-environment RDA")
...
windows(title="MSO plot of the detrended mite-environment RDA controlling for MEM")
msoplot(mite.env.MEM.mso, alpha=0.05/7)
mite.env.MEM.mso
